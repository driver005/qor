...
import (
	...
	"github.com/driver005/commerce/auth/claims"
...
// Config auth config
type Config struct {
	// Default Database, which will be used in Auth when do CRUD, you can change a request's DB isntance by setting request Context's value, refer https://github.com/driver005/commerce/auth/blob/master/utils.go#L32
	DB *gorm.DB
	// AuthIdentityModel a model used to save auth info, like email/password, OAuth token, linked user's ID, https://github.com/driver005/commerce/auth/blob/master/auth_identity/auth_identity.go is the default implemention
	...
	// Auth is using [Render](https://github.com/driver005/commerce/render) to render pages, you could configure it with your project's Render if you have advanced usage like [BindataFS](https://github.com/driver005/commerce/bindatafs)
	Render *render.Render
	// Auth is using [Mailer](https://github.com/driver005/commerce/mailer) to send email, by default, it will print email into console, you need to configure it to send real one
	...
	// LoginHandler defined behaviour when request `{Auth Prefix}/login`, default behaviour defined in http://godoc.org/github.com/driver005/commerce/auth#pkg-variables
	LoginHandler func(*Context, func(*Context) (*claims.Claims, error))
	// RegisterHandler defined behaviour when request `{Auth Prefix}/register`, default behaviour defined in http://godoc.org/github.com/driver005/commerce/auth#pkg-variables
	RegisterHandler func(*Context, func(*Context) (*claims.Claims, error))
	// LogoutHandler defined behaviour when request `{Auth Prefix}/logout`, default behaviour defined in http://godoc.org/github.com/driver005/commerce/auth#pkg-variables
...
func New(config *Config) *Auth {
	...
	config.Render.RegisterViewPath("github.com/driver005/commerce/auth/views")

...
func New(config *auth.Config) *auth.Auth {
	...
	config.ViewPaths = append(config.ViewPaths, "github.com/driver005/commerce/auth_themes/clean/views")
	...
	if config.Render == nil {
		...
		for _, gopath := range append([]string{filepath.Join(utils.AppRoot, "vendor")}, utils.GOPATH()...) {
			filePath := filepath.Join(gopath, "src", "github.com/driver005/commerce/auth_themes/clean/locales/en-US.yml")
